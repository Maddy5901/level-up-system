/**
 * Login Unit Tests - CORRECTED VERSION
 * Tests for login() function from auth.service.ts
 * Coverage: Input validation, credential verification, session handling, error cases
 *
 * Mock architecture: Uses vi.hoisted() to create shared mock functions that are
 * returned by createClient() on EVERY call, ensuring login() uses the same mocked
